...
try:
    # Use the original transformer block directly (no wrapper)
    ...
    # Prepare mask for fixed 32 length [32, 32]
    row_indices = torch.arange(32, device=device).unsqueeze(1)
    col_indices = torch.arange(32, device=device).unsqueeze(0)
    mask = (row_indices < col_indices)
    
    # Use RoPE parameters from the original model [32, head_dim]
    cos = model.cos  # Already on the correct device
    sin = model.sin  # Already on the correct device

    # Define ONNX export path
    onnx_path = Path(output_path, "qwen3_0.6b_transformer_block_0.onnx")

    # Export to ONNX with 4 inputs: (x, mask, cos, sin)
    torch.onnx.export(
        first_block,
        (dummy_input, mask, cos, sin),  # 4 input arguments
        str(onnx_path),
        input_names=['embeddings', 'mask', 'cos', 'sin'],
        output_names=['output'],
        dynamic_axes={
            'embeddings': {0: 'batch_size'},
            'output': {0: 'batch_size'}
            # mask, cos, sin are fixed size, so no dynamic axes needed
        },
        opset_version=11,
    ...
    print("Note: The exported ONNX model requires 4 inputs: embeddings, mask, cos, sin")
